...
from .metricsV4_3 import metric_class
...
def get_eccd_results(l_p,save_results=False,split_calculation=True):
    ...
    col = ['centremost','sensor','exp','inputs','filter_lossf','gen','set_type']
    total_t = Timer()
    t = Timer()
    ...
    for init,p in enumerate(l_p):
        ...
        for i,g in enumerate(l_gen):
            ...
            #to save scores
            c = p_exp['config']
            score_row_info = pd.DataFrame([[c['centremost'],c['sensor'],c['exp'],c['inputs'],c['fil_lossf'],g,c['set_type']]],columns=col)
            is_run_evaluation = True
            if is_run_evaluation:
                if split_calculation:
                    ...
                    print('Calculating: {} gen {}'.format(p_exp['exp_print'],g))
...
def worker(batch_size=0):
    ...
    while True:
        ...
        if step == 'new_inputs':
            ...
            sub_predictions = fit.evaluate_agent_fitness(arch_fname,w_fname=adjw_fname)
            ...
            fit.set_loss_f(loss_f)
            fit.set_dataset(inputs.clone(),target)
            predictions = fit.evaluate_agent_fitness(arch_fname,w_fname=adjw_fname)
