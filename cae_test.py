...
def generate_p_list(args):
    seeds    = [123,777,555,10331,7575]
    l_exp    = [1]#[1,2,3]
    # l_inputs  = [100]#[10,20,30,50,100]
    # l_filter = ['none','mio','softmax','mio','softmax'] # smooth lossf
    # l_loss_f = ['mae','mae','mae','bce','bce']          # smooth lossf
    # l_filter = ['none','mio','softmax','none','mio','softmax','none','mio','softmax']#['none','mio','softmax','mio','softmax']
    # l_loss_f = ['bin_f1','bin_f1','bin_f1','bin_ap','bin_ap','bin_ap','bin_hl','bin_hl','bin_hl']#['mae','mae','mae','bce','bce']
    
    p,stdout = init_p(args)

    l_channels  = ['s0']#['A-5']
    l_diff_type = ['non-diff']#['diff','non-diff']
    l_inputs  = [10,30]#[10,20,30,50,100]
    ...
    for channel in l_channels:
        for diff_type in l_diff_type:
            if diff_type == 'non-diff':
                ...
                l_filter = ['none','mio','softmax','mio','softmax']#['none','mio','softmax']
                l_loss_f = ['mae','mae','mae','mul_ce','mul_ce']#['mse','mse','mse']
            ...
            for exp in l_exp:
                for inputs in l_inputs:
                    for filter,loss_f in zip(l_filter,l_loss_f):
                        ...
                        if 'shms' in p['experiment_path']:
                            p_exp['experiment_path'] += 'shms_{}_{}/'.format(channel,diff_type)
                            p_exp['data_path'] += '{}_20Hz_TEset_nonNorm_addedMissSqSat.csv'.format(channel)
                        elif 'smap' in p['experiment_path']:
                            p_exp['experiment_path'] += 'smap_{}_{}/'.format(channel,diff_type)
                        ...
                        p_exp['exp_print'] = '{} -> input size {} filter {}-{} and seed: {}'\
                          .format(exp_name,inputs,filter,loss_f,p_exp['seed'])
