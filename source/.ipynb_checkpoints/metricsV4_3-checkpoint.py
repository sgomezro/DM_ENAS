...
class metric_class():
    ...
    def set_class_values(self,target,preds):
        if self.task_type == 'binary_classification':
            self.target = target[:,self.target_col].flatten().int()
    ...
    def get_bin_score_report(self): #needs revision
        ...
        '''
        scores =  np.array([auroc(self.predict,self.target,task='binary'),
                    average_precision(self.predict,self.target,task='binary'),
                    precision(self.predict,self.target,task='binary'),
                    recall(self.predict,self.target,task='binary'),
                    f1_score(self.predict,self.target,task='binary') ])
