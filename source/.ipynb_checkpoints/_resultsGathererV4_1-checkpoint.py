...
from .metricsV4_2 import metric_class
...
def get_eccd_results(l_p,save_results=False):
    ...
    col = ['centremost','sensor','exp','inputs','filter_lossf','gen']
    total_t = Timer()
    t = Timer()
    ...
    for init,p in enumerate(l_p):
        ...
        if p['is_new_inputs']:
            ...
            test_inputs,test_target = dataset.testset()
            scat_fnames = split_arrays(test_inputs,test_target,p['n_slaves'],tmp_folder)
            step = 'new_inputs'
        ...
        for i,g in enumerate(l_gen):
            ...
            if is_run_evaluation:
                ...
                pred_time = t.get_time_minutes()
                
                print('Calculating: {} gen {} sensor {}'.format(p_exp['exp_print'],g,c['sensor']))
                ...
                score_report['elapsed_pred'] = pred_time
                score_report['elapsed_score']= t.get_time_minutes()-pred_time
...
def worker(batch_size=0):
    ...
    # fit_test = fitness_functions(p,rank,set_type='testset')
    ...
    while True:
        ...
        if step == 'new_inputs':
            ...
            fit_test.set_dataset(inputs,target)

        elif step == 'calculate_fitness':
            arch_fname,adjw_fname = comm.bcast(None,root=0)
            sub_predictions = fit_test.evaluate_agent_fitness(arch_fname,w_fname=adjw_fname)
...
def split_arrays(inputs, targets, n_workers, temp_dir_path):
