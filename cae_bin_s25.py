...
def generate_p_list(args):
    ...
    l_wsize  = [2,5,10,20,30,50]

        
    p,stdout = init_p(args)
    if 'smap' in p['experiment_path']:
        dataset = 'smap'
        l_centers  = ['T-3']#['E-8','T-3','D-6','E-11']
    elif 'msl' in p['experiment_path']:
        dataset = 'msl'
        l_centers  = ['D-16']#['M-5','M-4','P-10','F-8']#['M-1','M-1','D-15','D-16']
    ...
    for centermost in l_centers:
        for diff_type in l_diff_type:
            if diff_type == 'non-diff':
                l_filter = ['mio','softmax']#['none','mio','softmax','none','mio','softmax']
                l_loss_f = ['bin_ap','bin_ap']#['bin_f1','bin_f1','bin_f1','bin_ap','bin_ap','bin_ap']
                opt_direction = 'maximize'
            elif diff_type == 'diff':
                l_filter = ['softmax','softmax']#['none','mio','softmax','mio','softmax']
                l_loss_f = ['mae','bin_ce']#['mae','mae','mae','bin_ce','bin_ce']
                opt_direction = 'minimize'

            for exp in l_exp:
                for w_size in l_wsize:
                    for filter,loss_f in zip(l_filter,l_loss_f):
                        ...
                        p_exp['exp_print'] = '{} -> {} centermost {} window size {} filter {}-{} and seed: {}'\
                          .format(exp_name,dataset,centermost,w_size,filter,loss_f,p_exp['seed'])
